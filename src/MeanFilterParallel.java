...
public class MeanFilterParallel extends RecursiveAction {
   ...
   public MeanFilterParallel(int[] OriginalPic, int initiate, int area, int[] newPic, int windowWidth ) {
   ...
   protected void computeDirectly() {
      ...
      for (int pos = Initiate; pos < Initiate + Area; pos++) {
         ...
         for (int t = -centre; t <= centre; t++) {
            ...
            red += (float) ((pixel & 0x00ff0000) >> 16) / sq;
            green += (float) ((pixel & 0x0000ff00) >> 8) / sq;
            blue += (float) ((pixel & 0x000000ff) >> 0) / sq;
         }
   ...
   protected void compute() {
      ...
      invokeAll(new MeanFilterParallel(original, Initiate, half, New, window),
             new MeanFilterParallel(original, Initiate + half, Area - half, New, window));
   }
   ...
   public static void main(String[] args) throws Exception {
      ...
      String picName = args[0];//pictre name
      String output = args[1];//new picture
      int windowWidth = Integer.parseInt(args[2]);//new picture
      ...
      ForkJoinPool pool = new ForkJoinPool();//invoke fork join
